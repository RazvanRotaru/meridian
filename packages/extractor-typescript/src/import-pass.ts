 ...
 * (`export * from`, `export { x } from`), inline import-type dependencies, and literal runtime
 * `import()` expressions. In-project targets receive a module->module `imports` edge; external
 * targets retain the imported binding when one exists.
 ...
 * Computed runtime imports, `require()`, and barrel-chain flattening remain out of scope — a barrel
 * surfacing as a hub is honest data. TypeScript's `import("pkg").Type` is an ImportTypeNode, not a
 * runtime import expression, and also belongs in this structural dependency pass.
 */

import type { SourceFile } from "ts-morph";
import { resolvedModuleFile } from "./import-reference";
import {
  externalBindingNames,
