...
describe("GraphSurface mount semantic-navigation parity", () => {
  ...
  it("mounts the review panel with an honest zero-match canvas", () => {
    const store = freshStore();
    store.setState({
      prSelected: 31,
      prReviewed: 31,
    ...
    expect(markup).toContain("No extracted graph matches this overview");
    expect(markup).toContain("Changed source is still available from the file list.");
    expect(markup).toContain("Files changed");
  });

  it("explains a filtered-empty review even while its invisible workspace sentinel remains", () => {
    const markup = renderEmptyReviewCanvas({
      seedIds: ["ts:packages/app/src/a.test.ts"],
      coverage: {
        contextId: "all-test-overview",
        pageIndex: 0,
        entries: [{
          index: 0,
          head: { state: "filtered", isTest: true },
    ...
    expect(markup).toContain('data-review-empty-canvas="true"');
    expect(markup).toContain("Test changes are hidden");
    expect(markup).toContain("Turn on Tests to show their extracted graph.");
  });

  it("directs a budget-deferred overview to an exact file graph", () => {
    const markup = renderEmptyReviewCanvas({
      coverage: {
        contextId: "deferred-overview",
        pageIndex: 0,
        entries: [{
          index: 0,
    ...
    expect(markup).toContain("This overview reached its graph budget");
    expect(markup).toContain("Select a changed file to load its exact graph.");
  });

  it("distinguishes included extracted nodes without a drawable overview card", () => {
    const markup = renderEmptyReviewCanvas({
      coverage: {
        contextId: "included-overview",
        pageIndex: 0,
        entries: [{
          index: 0,
          head: { state: "included", isTest: false },
          mergeBase: { state: "absent", isTest: null },
          isTest: false,
        }],
      },
    });

    expect(markup).toContain("Extracted nodes are available without an overview card");
    expect(markup).toContain("Select a changed file to load its exact graph.");
  });

  it("uses file-specific no-match copy for an exact coordinate", () => {
    const markup = renderEmptyReviewCanvas({
      cursor: "file:0",
      coverage: {
        contextId: "retained-overview",
        pageIndex: 0,
        entries: [{
          index: 0,
          head: { state: "filtered", isTest: true },
    ...
    expect(markup).toContain("No extracted graph matches this file");
    expect(markup).toContain("Changed source is still available from the file list.");
    expect(markup).not.toContain("Test changes are hidden");
  });
});

function renderEmptyReviewCanvas({
  coverage = null,
  cursor = null,
  seedIds = [],
}: {
  ...
  store.setState({
    ...
    prPreparedOverviewCoverage: coverage,
    minimalSeedIds: seedIds,
    minimalMemberIds: [],
    minimalRfNodes: [],
    minimalRfEdges: [],
    minimalLayoutStatus: "idle",
    review: {
      context: {
        ...
        baseSha: "base",
        headRef: "feature",
        reviewKey: coverage?.contextId ?? "empty-review",
        warnings: [],
      },
      rows: [],
      flows: {},
