...
const wire = (id: string, source: string, target: string, kind: string, opts: { weight?: number; opacity?: number; type?: string } = {}): Edge => ({
  id,
  source,
  target,
  type: opts.type,
  data: { depKind: kind, category: "dep", weight: opts.weight ?? 1, underlyingEdgeIds: [id] },
  style: { opacity: opts.opacity ?? 0.4, stroke: "#123456" },
  markerEnd: `marker-${id}`,
});
