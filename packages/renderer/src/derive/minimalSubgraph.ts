/**
 * The minimal containment subtree for ANY set of seed file modules: the ancestor union over every
 * seed, PLUS each seed's DIRECT 1-hop import neighbors in BOTH directions (files it imports =
...
function addPair(
  ...
  if (!pairs.has(key)) {
    pairs.set(key, { id: `min:${key}`, source, target, weight: graph.weight.get(weightKey(source, target)) ?? 1 });
  }
}
