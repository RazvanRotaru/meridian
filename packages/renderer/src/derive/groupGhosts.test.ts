...
function emission(): GhostEmission {
  ...
  return {
    ...
    wires: [
      { source: "ts:t/host/h1.ts#f1", target: "vis", weight: 1, kind: "calls" },
      { source: "ts:t/host/h2.ts#f2", target: "vis", weight: 2, kind: "calls" },
      { source: "ts:t/host/h3.ts#f3", target: "vis", weight: 1, kind: "references" },
      { source: "ts:t/lone/l.ts#g", target: "vis", weight: 1, kind: "calls" },
    ],
  };
}
...
describe("groupGhostEmission", () => {
  ...
  it("folds a PAIR from the same folder (threshold 2) but never a lone ghost", () => {
    ...
    const input: GhostEmission = {
      ...
      wires: [
        { source: "ts:t/host/h1.ts#f1", target: "vis", weight: 1, kind: "calls" },
        { source: "ts:t/host/h2.ts#f2", target: "vis", weight: 1, kind: "calls" },
      ],
    };
    const grouped = groupGhostEmission(input, indexFixture());
    expect([...grouped.ghosts.keys()]).toEqual(["ts:t/host"]);
    expect(grouped.wires).toEqual([{ source: "ts:t/host", target: "vis", weight: 2, kind: "calls" }]);
  });
  ...
  it("returns the emission unchanged when no folder reaches the threshold", () => {
    const input: GhostEmission = {
      ...
      wires: [{ source: "ts:t/host/h1.ts#f1", target: "vis", weight: 1, kind: "calls" }],
    };
